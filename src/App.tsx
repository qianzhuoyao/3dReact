...
function App() {
  ...
  useEffect(() => {
    const subscription = getWindowSingle().singals.singalUpdate.subscribe(
      (data) => {
        if (data.messageCode === "deviceCheck") {
          console.log(data, "dataaewee");
          setDeviceInfo(data.payload as IDevice);
          setState(() => {
            if (data.payload) {
              return "deviceCheck";
            } else {
          ...
          if (data.payload) {
            fetch("mwapi/alert/thirdAlert/thirdCountCurrentAlertLevelCount", {
              method: "POST",
              headers: {
            ...
            fetch("/mwapi/alert/thirdAlert/thirdCurrent", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
              body: JSON.stringify({
                ...
                instanceIdList: [(data.payload as IDevice).id],
                pageNum: 1,
                pageSize: 9999999,
              }),
            })
                  ...
                  {[
                    ...
                    return (
                      <div key={item.label}>
                        ...
                        >
                          {deviceInfo?.[item.value as keyof IDevice] ?? ""}
                        </span>
                      </div>
                    );
                  })}
                  ...
                  {[
                    {
                      label: "告警总数",
                      icon: alertCount,
                      value: alertInfo?.total,
                    },
                    {
                      label: "最近发生告警时间",
                      icon: alertTime,
                      value: alertInfo?.records?.[0]?.eventTime,
                    },
                  ].map((item) => (
                    <div
                      key={item.label}
